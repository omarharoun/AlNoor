...
func TestTenorTrending(t *testing.T) {
	client := newTestClient(t)
	user := createTestAccount(t, client)

	t.Run("can get trending GIFs", func(t *testing.T) {
		trendingResp := getTenorTrending(t, client, user.Token, "")
	...
	t.Run("can get trending with locale", func(t *testing.T) {
		trendingResp := getTenorTrending(t, client, user.Token, "fr")
	...
	t.Run("requires authentication", func(t *testing.T) {
		resp, err := client.get("/tenor/trending-gifs")
		if err != nil {
